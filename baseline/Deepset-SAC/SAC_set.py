...
class SoftQNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, init_w=3e-3):
        ...
        ## Q值网络的action和obs在embedding 层后拼接
        # self.linear1 = nn.Linear(num_inputs + num_actions, hidden_size)
        # self.linear2 = nn.Linear(hidden_size, hidden_size)
        # self.linear3 = nn.Linear(hidden_size, hidden_size)
        ## 假设最大10个包裹
        self.embedding1 = nn.Linear(5,hidden_size)
        self.embedding2 = nn.Linear(5,hidden_size)
        self.embedding3 = nn.Linear(5,hidden_size)
        self.embedding4 = nn.Linear(5,hidden_size)
        self.embedding5 = nn.Linear(5,hidden_size)
        self.embedding6 = nn.Linear(5,hidden_size)
        self.embedding7 = nn.Linear(5,hidden_size)
        self.embedding8 = nn.Linear(5,hidden_size)
        self.embedding9 = nn.Linear(5,hidden_size)
        # self.embedding10 = nn.Linear(5,hidden_size)
        self.action_embedding = nn.Linear(num_actions,hidden_size)
    ...
    def forward(self, state, action):
        state = torch.cat((state,torch.zeros(state.size(0),5).to(device)),1)
        x1 = self.embedding1(state[:,0:5])
        x2 = self.embedding1(state[:,5:10])
        x3 = self.embedding1(state[:,10:15])
        x4 = self.embedding1(state[:,15:20])
        x5 = self.embedding1(state[:,20:25])
        x6 = self.embedding1(state[:,25:30])
        x7 = self.embedding1(state[:,30:35])
        x8 = self.embedding1(state[:,35:40])
        x9 = self.embedding1(state[:,40:45])
        # x10 = self.embedding1(state[:,45:50])

        x_actiion = F.relu(self.action_embedding(action))
        x = F.relu(torch.sum(torch.cat([
        torch.unsqueeze(x1,2),
        torch.unsqueeze(x2,2),
        torch.unsqueeze(x3,2),
        torch.unsqueeze(x4,2),
        torch.unsqueeze(x5,2),
        torch.unsqueeze(x6,2),
        torch.unsqueeze(x7,2),
        torch.unsqueeze(x8,2),
        torch.unsqueeze(x9,2)],
        2),2))

        ## action 和 obs 拼接成1024
        x = torch.cat([x, x_actiion], 1)  
...
class PolicyNetwork(nn.Module):     ## input obs ;output action
...
writer = SummaryWriter('./logs/sac_set_3')
tmp_avg_reward = 0
